...
std::optional<std::shared_ptr<ControllerServiceType>> parseOptionalControllerService(const core::ProcessContext& context, const core::PropertyReference& prop) {
  const auto controller_service_name = context.getProperty(prop.name);
  if (!controller_service_name) {
    return std::nullopt;
  }

  const std::shared_ptr<core::controller::ControllerService> service = context.getControllerService(*controller_service_name);
  if (!service) {
    return std::nullopt;
  }
