...
class ProcessContext {
 ...
 protected:
  [[nodiscard]] virtual std::expected<std::string, std::error_code> getProperty(std::string_view name, const FlowFile* flow_file) const = 0;
};
...
class CffiProcessContext : public ProcessContext {
 ...
 protected:
  [[nodiscard]] std::expected<std::string, std::error_code> getProperty(std::string_view name, const FlowFile* flow_file) const override;
