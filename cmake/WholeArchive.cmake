...
function(target_wholearchive_library_public TARGET ITEM)
    if (APPLE)
        target_link_libraries(${TARGET} PUBLIC ${ITEM})
        target_link_libraries(${TARGET} PUBLIC -Wl,-force_load,$<TARGET_FILE:${ITEM}>)
        add_dependencies(${TARGET} ${ITEM})
    elseif(WIN32)
        target_link_libraries(${TARGET} PUBLIC ${ITEM})
        if(${CMAKE_VERSION} VERSION_LESS "3.13.0")
            ...
            target_link_options(${TARGET} PUBLIC "/WHOLEARCHIVE:$<TARGET_FILE:${ITEM}>")
        endif()
        add_dependencies(${TARGET} ${ITEM})
    else()
        target_link_libraries(${TARGET} PUBLIC -Wl,--whole-archive ${ITEM} -Wl,--no-whole-archive)
        add_dependencies(${TARGET} ${ITEM})
    endif()
endfunction(target_wholearchive_library_public)
