...
from minifi_test_framework.containers.MinifiContainer import MinifiContainer
...
def common_before_scenario(context: Context, scenario: Scenario):
    ...
    context.scenario_id = scenario.filename.rsplit("/", 1)[1].split(".")[0] + "-" + str(scenario.parent.scenarios.index(scenario))
    network_name = f"{context.scenario_id}-net"
    docker_client = docker.client.from_env()
    try:
        existing_network = docker_client.networks.get(network_name)
        logging.warn(f"Found existing network '{network_name}'. Removing it first.")
...
def common_after_scenario(context: MinifiTestContext, _scenario: Scenario):
