...
class WindowsContainer(ContainerProtocol):
    ...
    def deploy(self) -> bool:
        ...
        try:
            ...
            for file in self.files:
                ...
                check_output(f"docker cp {temp_path} {self.container_name}:C:{file.path}", shell=True, stderr=subprocess.STDOUT)
    ...
    def verify_file_contents(self, directory_path: str, expected_contents: list[str]) -> bool:
        ...
        logging.info(f"Verifying {directory_path}, expected: {expected_contents}")
        ...
        exit_code, output = self.exec_run(list_files_command)
        ...
        for path in actual_filepaths:
            # Read raw content, normalize line endings, and trim
            read_command = f"powershell -NonInteractive -NoProfile -Command \"$c = Get-Content -Path '{path}' -Raw; ($c -replace '\\r\\n', '\\n' -replace '\\r', '\\n').Trim()\""
            ...
            actual_file_contents.append(content) # Already trimmed and normalized

        # Normalize the expected contents list for comparison
        normalized_expected_contents = [
            s.strip().replace("\r\n", "\n").replace("\r", "\n") for s in expected_contents
        ]

        return sorted(actual_file_contents) == sorted(normalized_expected_contents)
