...
class WindowsContainer(ContainerProtocol):
    ...
    def directory_contains_file_with_content(self, directory_path: str, expected_content: str) -> bool:
        ...
        command = f"C:\\Program Files\\Git\\bin\\sh.exe -c \"grep -l -F -- '{escaped_content}' {directory_path}/*\""
    ...
    def verify_file_contents(self, directory_path: str, expected_contents: list[str]) -> bool:
        ...
        actual_filepaths = [path for path in output.splitlines() if path]
        ...
        for path in actual_filepaths:
            ...
            exit_code, output = self.exec_run(read_command)

            if exit_code != 0:
                logging.error(f"Command to read file '{path}' failed with exit code {exit_code}")
                return False

            actual_file_contents.append(output)
