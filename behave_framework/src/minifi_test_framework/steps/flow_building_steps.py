...
def step_impl(context: MinifiTestContext, processor_name: str, relationship: str):
    context.minifi_container.flow_definition.get_processor(processor_name).auto_terminated_relationships.append(relationship)
...
def step_impl(context: MinifiTestContext):
    for row in context.table:
        ...
        if dest_proc_name == "auto-terminated":
            context.minifi_container.flow_definition.get_processor(source_proc_name).auto_terminated_relationships.append(relationship)
        else:
            connection = Connection(source_name=row["source name"], source_relationship=relationship, target_name=row["destination name"])
            context.minifi_container.flow_definition.add_connection(connection)
