...
def step_impl(context: MinifiTestContext, content: str, directory: str, duration: str):
    ...
    assert wait_for_condition(condition=lambda: context.minifi_container.directory_has_single_file_with_content(directory, new_content),
...
def step_impl(context: MinifiTestContext):
    assert "[error]" not in context.minifi_container.get_logs()

@then("the Minifi logs do not contain warnings")
def step_impl(context: MinifiTestContext):
    assert "[warning]" not in context.minifi_container.get_logs()
...
def step_impl(context: MinifiTestContext, url: str):
    ...
    assert http_proxy_container.check_http_proxy_access(url)
...
def step_impl(context: MinifiTestContext, directory: str, regex_str: str, duration: str):
    duration_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(condition=lambda: context.minifi_container.directory_contains_file_with_regex(directory, regex_str),
                              timeout_seconds=duration_seconds,
                              bail_condition=lambda: context.minifi_container.exited)
