...
void PublishMQTT::onTriggerImpl(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  if (record_converter_) {
    ...
    session.read(original_flow_file, [this, &record_set](const std::shared_ptr<io::InputStream>& input_stream) -> io::ExpectedCallbackReturn {
      record_set = record_converter_->record_set_reader->read(*input_stream);
      return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(input_stream->size()));
    });
