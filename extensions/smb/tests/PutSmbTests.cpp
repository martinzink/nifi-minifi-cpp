...
TEST_CASE("PutSmb conflict resolution test") {
  ...
  SECTION("Replace") {
    ...
    CHECK(first_trigger_results.at(PutSmb::Failure).size() == 0);
    ...
    CHECK(second_trigger_results.at(PutSmb::Failure).size() == 0);
  ...
  SECTION("Fail") {
    ...
    CHECK(second_trigger_results.at(PutSmb::Success).size() == 0);
