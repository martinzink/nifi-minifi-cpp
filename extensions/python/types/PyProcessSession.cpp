...
void PyProcessSession::read(const std::shared_ptr<core::FlowFile>& flow_file, BorrowedObject input_stream_callback) {
  ...
  session_.read(flow_file, [&input_stream_callback](const std::shared_ptr<io::InputStream>& input_stream) -> io::ExpectedCallbackReturn {
    return io::i64ToExpectedCallbackReturn(Long(Callable(input_stream_callback.getAttribute("process"))(std::weak_ptr(input_stream))).asInt64());
  });
}
...
void PyProcessSession::write(const std::shared_ptr<core::FlowFile>& flow_file, BorrowedObject output_stream_callback) {
  ...
  session_.write(flow_file, [&output_stream_callback](const std::shared_ptr<io::OutputStream>& output_stream) -> io::ExpectedCallbackReturn {
    return io::i64ToExpectedCallbackReturn(Long(Callable(output_stream_callback.getAttribute("process"))(std::weak_ptr(output_stream))).asInt64());
  });
}
...
std::string PyProcessSession::getContentsAsString(const std::shared_ptr<core::FlowFile>& flow_file) {
  ...
  std::string content;
  session_.read(flow_file, [&content](const std::shared_ptr<io::InputStream>& input_stream) -> io::ExpectedCallbackReturn {
    content.resize(input_stream->size());
    return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(input_stream->read(as_writable_bytes(std::span(content)))));
  });
  return content;
}
