...
void FlowFileSource::FlowFileGenerator::endProcessBatch() {
  ...
  new_flow->addAttribute(FragmentIndex.name, std::to_string(flow_files_.size()));
  new_flow->addAttribute(std::string{FlowFileSource::FragmentIdentifier.name}, batch_id_.to_string());
...
void FlowFileSource::FlowFileGenerator::finishProcessing() {
  ...
  for (const auto& flow_file : flow_files_) {
    flow_file->addAttribute(FragmentCount.name, fragment_count);
  }
}
