...
void FetchAzureBlobStorage::onTrigger(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  session.write(fetched_flow_file, [&, this](const std::shared_ptr<io::OutputStream>& stream) -> io::ExpectedCallbackReturn {
    result_size = azure_blob_storage_.fetchBlob(*params, *stream);
    if (!result_size) {
      return 0;
    }
    return io::i64ToExpectedCallbackReturn(*result_size);
  });
