...
void SplitJson::onTrigger(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  for (size_t i = 0; i < result_array.size(); ++i) {
    ...
    session.write(child_flow_file, [this, &json_value_to_write](const std::shared_ptr<io::OutputStream>& output_stream) -> io::ExpectedCallbackReturn {
      auto result_string = jsonValueToString(json_value_to_write);
      return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(output_stream->write(reinterpret_cast<const uint8_t*>(result_string.data()), result_string.size())));
    });
