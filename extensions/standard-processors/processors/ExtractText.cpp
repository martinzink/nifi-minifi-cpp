...
int64_t ExtractText::ReadCallback::operator()(const std::shared_ptr<io::InputStream>& stream) const {
  ...
  if (regex_mode) {
    ...
    const bool repeatingcapture = utils::parseBoolProperty(*ctx_, EnableRepeatingCaptureGroup);
    const size_t max_capture_size = gsl::narrow<size_t>(utils::parseU64Property(*ctx_, MaxCaptureGroupLen));
