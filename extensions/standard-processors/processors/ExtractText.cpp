...
io::ExpectedCallbackReturn ExtractText::ReadCallback::operator()(const std::shared_ptr<io::InputStream>& stream) const {
  ...
  while (read_size < size_limit) {
    ...
    if (io::isError(ret)) {
      return nonstd::make_unexpected(MINIFI_IO_ERROR);  // Stream error
    } else if (ret == 0) {
  ...
  return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(read_size));
}
