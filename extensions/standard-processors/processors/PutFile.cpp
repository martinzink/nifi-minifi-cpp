...
void PutFile::onSchedule(core::ProcessContext *context, core::ProcessSessionFactory* /*sessionFactory*/) {
  conflict_resolution_strategy_ = utils::parseEnumProperty<put_file::FileExistsResolutionStrategy>(*context, ConflictResolution);
...
void PutFile::onTrigger(core::ProcessContext *context, core::ProcessSession *session) {
  ...
  if (utils::file::exists(*dest_path)) {
    ...
    if (conflict_resolution_strategy_ == put_file::FileExistsResolutionStrategy::FAIL_FLOW) {
      return session->transfer(flow_file, Failure);
    } else if (conflict_resolution_strategy_ == put_file::FileExistsResolutionStrategy::IGNORE_REQUEST) {
