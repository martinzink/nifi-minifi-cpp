...
void HashContent::onTrigger(core::ProcessContext&, core::ProcessSession& session) {
  ...
  session.read(flowFile, [&flowFile, this](const std::shared_ptr<io::InputStream>& stream) -> io::ExpectedCallbackReturn {
    const auto& ret_val = algorithm_(stream);
    flowFile->setAttribute(attrKey_, ret_val.first);

    return io::i64ToExpectedCallbackReturn(ret_val.second);
  });
