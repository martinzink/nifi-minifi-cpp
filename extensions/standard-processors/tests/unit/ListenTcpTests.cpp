...
TEST_CASE("Test ListenTCP with SSL connection", "[ListenTCP]") {
  ...
  SECTION("With client certificate provided") {
    ...
    expected_successful_messages = {"test_message_1", "another_message"};
    for (const auto& message : expected_successful_messages) {
      auto send_error = utils::sendMessagesViaSSL({message}, endpoint, minifi::utils::file::FileUtils::get_executable_dir() + "/resources/ca_A.crt", ssl_data);
  ...
  SECTION("Required certificate not provided") {
    ...
    auto send_error = utils::sendMessagesViaSSL({"test_message_1"}, endpoint, minifi::utils::file::concat_path(executable_dir, "/resources/ca_A.crt"));
    CHECK(send_error);
  }
