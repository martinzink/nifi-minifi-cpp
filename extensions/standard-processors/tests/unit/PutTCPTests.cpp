...
class CancellableTcpServer : public utils::net::TcpServer {
 public:
  ...
  asio::awaitable<void> listen() override {
    ...
    while (true) {
      ...
      auto& cancellable_timer = cancellable_timers_.emplace_back(io_context_);
      if (ssl_data_)
 ...
 private:
  asio::awaitable<void> wait_until_cancelled(asio::steady_timer& timer) {
    timer.expires_at(asio::steady_timer::time_point::max());
    co_await timer.async_wait(utils::net::use_nothrow_awaitable);
  }

  std::vector<asio::steady_timer> cancellable_timers_;
};
