...
constexpr auto server_startup_wait_time = 10ms;
...
TEST_CASE("Test ListenSyslog via TCP with SSL connection", "[ListenSyslog]") {
  ...
  controller.plan->scheduleProcessor(listen_syslog);
  std::this_thread::sleep_for(server_startup_wait_time);
  check_no_error(utils::sendMessagesViaSSL({rfc5424_logger_example_1}, endpoint, minifi::utils::file::concat_path(executable_dir, "resources/ca_A.crt")));
  check_no_error(utils::sendMessagesViaSSL({invalid_syslog}, endpoint, minifi::utils::file::concat_path(executable_dir, "/resources/ca_A.crt")));
