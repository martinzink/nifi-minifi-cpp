...
TEST_CASE("JoinEnrichmentAttributes input without appropriate attributes") {
  ...
  CHECK(trigger_result.at(JoinEnrichmentAttributes::TimeoutRelationship).size() == 0);
...
TEST_CASE("JoinEnrichmentAttributes same id same role different session") {
  ...
  CHECK(second_trigger.at(JoinEnrichmentAttributes::TimeoutRelationship).size() == 0);
