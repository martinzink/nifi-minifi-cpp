...
void JsonRecordSetWriter::writePerLine(const core::RecordSet& record_set, const std::shared_ptr<core::FlowFile>& flow_file, core::ProcessSession& session) {
  session.write(flow_file, [&record_set](const std::shared_ptr<io::OutputStream>& stream) -> io::ExpectedCallbackReturn {
    ...
    return io::i64ToExpectedCallbackReturn(write_result);
  });
}
...
void JsonRecordSetWriter::writeAsArray(const core::RecordSet& record_set, const std::shared_ptr<core::FlowFile>& flow_file, core::ProcessSession& session) const {
  session.write(flow_file, [this, &record_set](const std::shared_ptr<io::OutputStream>& stream) -> io::ExpectedCallbackReturn {
    ...
    return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(stream->write(gsl::make_span(fmt::format("{}", buffer.GetString())).as_span<const std::byte>())));
  });
}
