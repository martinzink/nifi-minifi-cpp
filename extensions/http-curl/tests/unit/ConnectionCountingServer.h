...
class NumberedMethodResponder : public CivetHandler {
 public:
  NumberedMethodResponder(std::set<utils::SmallString<36>>& connections) : connections_(connections) {}
 ...
 private:
  ...
  void saveConnectionId(struct mg_connection *conn) {
    auto user_data = reinterpret_cast<utils::SmallString<36>*>(mg_get_user_connection_data(conn));
    connections_.emplace(*user_data);
  }
...
class ReverseBodyPostHandler : public CivetHandler {
 public:
  ReverseBodyPostHandler(std::set<utils::SmallString<36>>& connections) : connections_(connections) {}
...
class ConnectionCountingServer {
 public:
  ...
  size_t getNumberOfConnections() { return connections_.size(); }
  std::string getPort() {
 ...
 private:
  ...
  std::set<utils::SmallString<36>> connections_;
  AddIdToUserConnectionData add_id_to_user_connection_data_;
  CivetServer server_{options, &add_id_to_user_connection_data_};
  ReverseBodyPostHandler reverse_body_post_handler_{connections_};
  NumberedMethodResponder numbered_method_responder_{connections_};
};
