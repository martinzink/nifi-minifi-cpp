...
void CaptureRTSPFrame::onTrigger(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  if (video_capture_.read(frame)) {
    if (!frame.empty()) {
      ...
      session.write(flow_file, [&frame, this](const std::shared_ptr<io::OutputStream>& output_stream) -> io::ExpectedCallbackReturn {
        ...
        return io::i64ToExpectedCallbackReturn(ret);
      });
