  ...
  static rd_kafka_headers_unique_ptr make_headers(const core::FlowFile& flow_file, const std::optional<utils::Regex>& attribute_name_regex) {
    ...
    for (const auto& [attribute_key, attribute_value]: flow_file.getAttributes()) {
      if (attribute_name_regex && utils::regexMatch(attribute_key, *attribute_name_regex)) {
        rd_kafka_header_add(result, attribute_key.c_str(), gsl::narrow<ssize_t>(attribute_key.size()), attribute_value.c_str(), gsl::narrow<ssize_t>(attribute_value.size()));
      }
    }
...
bool PublishKafka::configureNewConnection(core::ProcessContext& context) {
  ...
  gsl::owner<rd_kafka_t*> producer{rd_kafka_new(RD_KAFKA_PRODUCER, conf_.release(), err_chars.data(), err_chars.size())};
  if (producer == nullptr) {
...
bool PublishKafka::createNewTopic(core::ProcessContext& context, const std::string& topic_name, const std::shared_ptr<core::FlowFile>& flow_file) const {
  ...
  const auto kafka_topic_ref = std::make_shared<KafkaTopic>(topic_reference);  // KafkaTopic takes ownership of topic_reference
