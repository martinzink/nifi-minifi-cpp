...
class ReadCallback {
  ...
  static rd_kafka_headers_unique_ptr make_headers(const api::core::FlowFile& flow_file,
      const api::core::ProcessSession& session,
      const std::optional<utils::Regex>& attribute_name_regex) {
    rd_kafka_headers_unique_ptr result{rd_kafka_headers_new(8)};
  ...
  rd_kafka_resp_err_t produce(const size_t segment_num, std::vector<std::byte>& buffer, const size_t buflen) const {
    ...
    auto hdrs_copy = rd_kafka_headers_unique_ptr{rd_kafka_headers_copy(hdrs.get())};
 ...
 public:
  ...
  const gsl::not_null<rd_kafka_headers_unique_ptr> hdrs;
...
void PublishKafka::configureNewConnection(api::core::ProcessContext& context) {
  ...
  rd_kafka_conf_unique_ptr conf{rd_kafka_conf_new()};
...
; }
  ...
  rd_kafka_conf_set_log_cb(conf.get(), &KafkaOpaque::logCallback);

  // The producer takes ownership of the configuration, we must not free it
  rd_kafka_producer_unique_ptr producer{
...
bool PublishKafka::createNewTopic(const api::core::ProcessContext& context, const std::string& topic_name,
  ...
  rd_kafka_topic_conf_unique_ptr topic_conf_{rd_kafka_topic_conf_new()};
  ...
  rd_kafka_topic_unique_ptr topic_reference{
