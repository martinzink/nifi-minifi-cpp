...
void setKafkaConfigurationField(rd_kafka_conf_t& configuration, const std::string_view field_name, const std::string_view value) {
  static std::array<char, 512U> errstr{};
  rd_kafka_conf_res_t result{};
  result = rd_kafka_conf_set(&configuration, field_name.data(), value.data(), errstr.data(), errstr.size());
...
void KafkaOpaque::rebalance_cb(rd_kafka_t* rk, rd_kafka_resp_err_t trigger, rd_kafka_topic_partition_list_t* partitions, void* opaque_ptr) {
  KafkaOpaque* kafka_opaque = static_cast<KafkaOpaque*>(opaque_ptr);
  if (!kafka_opaque) {
    return;
  }
...
std::string get_human_readable_kafka_message_headers(const rd_kafka_message_t& rkmessage, core::logging::Logger& logger) {
  ...
  if (RD_KAFKA_RESP_ERR_NO_ERROR == get_header_response) {
    ...
    return minifi::utils::string::join(", ", header_list);
  }
...
std::string get_encoded_string(const std::string& input, KafkaEncoding encoding) {
  switch (encoding) {
    ...
    case KafkaEncoding::HEX: return minifi::utils::string::to_hex(input, /* uppercase = */ true);
  }
