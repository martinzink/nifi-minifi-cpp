...
void PutS3Object::onTrigger(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  session.read(flow_file, [this, &flow_file, &put_s3_request_params, &result](const std::shared_ptr<io::InputStream>& stream) -> io::ExpectedCallbackReturn {
    try {
      if (flow_file->getSize() <= multipart_threshold_) {
        ...
        return io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(flow_file->getSize()));
      }
