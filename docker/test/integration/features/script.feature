...
Feature: MiNiFi can execute Lua and Python scripts
  ...
  Scenario: ExecuteScript should only allow the number of parallel tasks defined by the max concurrent tasks attribute for Lua scripts
    Given a ExecuteScript processor with the "Script File" property set to "/tmp/resources/lua/sleep_forever.lua"
    And the scheduling period of the ExecuteScript processor is set to "500 ms"
  ...
  Scenario: ExecuteScript should only allow one Python script running at a time
    Given a ExecuteScript processor with the "Script File" property set to "/tmp/resources/python/sleep_forever.py"
    And the scheduling period of the ExecuteScript processor is set to "500 ms"
