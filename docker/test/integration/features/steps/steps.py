...
def step_impl(context, producer_name, consumer_name):
    minifi_crt_file = f'/tmp/resources/minifi_client.crt'
    minifi_key_file = f'/tmp/resources/minifi_client.key'
    root_ca_crt_file = f'/tmp/resources/root_ca.crt'
    ssl_context_service = SSLContextService(cert=minifi_crt_file, ca_cert=root_ca_crt_file, key=minifi_key_file)

    secondary_cert, secondary_key = make_cert(context.test.get_container_name_with_postfix("secondary"), context.test.root_ca_cert, context.test.root_ca_key)
    secondary_pem_file = '/tmp/resources/secondary.crt'
    context.test.put_test_resource('secondary.crt', dump_certificate(secondary_cert) + dump_privatekey(secondary_key))
...
def step_impl(context):
    ...
    splunk_cert, splunk_key = make_cert(context.test.get_container_name_with_postfix("splunk"), context.test.root_ca_cert, context.test.root_ca_key)
    ...
    context.test.enable_splunk_hec_ssl('splunk', dump_certificate(splunk_cert), dump_privatekey(splunk_key), dump_certificate(context.test.root_ca_cert))
...
def step_impl(context, content, topic_name):
    ...
    ca_cert_file.write(dump_certificate(context.test.root_ca_cert))
    ca_cert_file.close()
    os.chmod(ca_cert_file.name, 0o644)

    client_cert, client_key = make_cert(socket.gethostname(), context.test.root_ca_cert, context.test.root_ca_key, is_server_auth=False)
    client_cert_file = tempfile.NamedTemporaryFile(delete=False)
    client_cert_file.write(dump_certificate(client_cert))
    client_cert_file.close()
    os.chmod(client_cert_file.name, 0o644)

    client_key_file = tempfile.NamedTemporaryFile(delete=False)
    client_key_file.write(dump_privatekey(client_key))
...
def step_impl(context, log_count, log_pattern):
    context.test.check_container_log_matches_regex('mqtt-broker', log_pattern, 30, count=int(log_count))
