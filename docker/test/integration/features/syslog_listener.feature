...
Feature: Minifi C++ can act as a syslog listener
  ...
  Scenario: A syslog client can send messages to Minifi over UDP
    ...
    And the "Replacement Value" property of the ReplaceText processor is set to "${syslog.timestamp:toDate():format('%Y-%m-%d %H:%M:%S', 'UTC')}	${syslog.priority}	${reverseDnsLookup(${syslog.sender}, 100)}	${syslog.msg}"
    ...
    Then at least one flowfile's content match the following regex: "^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}\t13\t.*\tsample_log$" in less than 10 seconds
