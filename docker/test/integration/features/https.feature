...
Feature: Using SSL context service to send data with TLS
  ...
  Scenario: A MiNiFi instance sends data using InvokeHTTP to a receiver using ListenHTTP with TLS
    ...
    And 100 files with the content "test" are present in "/tmp/input"
    ...
    Then 100 flowfiles with the content "test" are placed in the monitored directory in less than 60 seconds
