...
from ssl_utils.SSL_cert_utils import make_cert
...
class ElasticsearchContainer(Container):
    def __init__(self, context, name, vols, network, image_store, command=None):
        ...
        http_cert, http_key = make_cert(f"elasticsearch-{context.feature_id}", context.test.root_ca_cert, context.test.root_ca_key, is_server_auth=True, is_client_auth=False)
        transport_cert, transport_key = make_cert(f"127.0.0.1", context.test.root_ca_cert, context.test.root_ca_key, is_server_auth=False, is_client_auth=False)
