...
class KafkaBrokerContainer(Container):
    def __init__(self, context, name, vols, network, image_store, command=None):
        ...
        kafka_cert, kafka_key = make_cert(f"kafka-broker-{context.feature_id}", context.test.root_ca_cert, context.test.root_ca_key, is_server_auth=True, is_client_auth=False)
