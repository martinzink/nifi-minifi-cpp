...
class MinifiC2ServerContainer(Container):
    def __init__(self, context, name, vols, network, image_store, command=None, ssl=False):
        ...
        self.ssl = ssl
        if ssl:
            c2_cert, c2_key = make_cert(f"minifi-c2-server-{context.feature_id}", context.test.root_ca_cert, context.test.root_ca_key, is_server_auth=True, is_client_auth=False)
