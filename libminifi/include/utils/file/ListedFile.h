...
class ListedFile : public utils::ListedObject {
 public:
  explicit ListedFile(std::filesystem::path full_file_path) : full_file_path_(std::move(full_file_path)) {
 ...
 private:
  bool matchesRegex(const std::optional<std::regex>& file_regex, const std::optional<std::regex>& path_regex) const {
    if (file_regex && !std::regex_match(full_file_path_.filename().string(), *file_regex))
      return false;
    if (path_regex && !std::regex_match(full_file_path_.parent_path().string(), *path_regex))
      return false;
    return true;
  }

  bool fileAgeIsBetween(const std::optional<std::chrono::milliseconds> minimum_age, const std::optional<std::chrono::milliseconds>  maximum_age) const {
  ...
  bool fileSizeIsBetween(const std::optional<size_t> minimum_size, const std::optional<size_t> maximum_size) const {
 ...
 private:
  std::chrono::system_clock::duration getAge() const { return std::chrono::system_clock::now() - last_modified_time_;}
  size_t getSize() const { return utils::file::file_size(full_file_path_); }
