...
void StructuredConfiguration::parsePropertyValueSequence(const std::string& property_name, const Node& property_value_node, core::ConfigurableComponent& component,
  ...
  for (const auto& nodeVal : property_value_node) {
    if (nodeVal) {
      ...
      if (!append_prop_result && append_prop_result.error() == PropertyErrorCode::NotSupportedProperty) {
...
void StructuredConfiguration::parseSingleProperty(const std::string& property_name, const Node& property_value_node, core::ConfigurableComponent& processor,
    ParameterContext* parameter_context) {
  auto my_prop = processor.getPropertyReference(property_name);
  ...
  if (my_prop) {
    const auto prop_set = processor.setProperty(property_name, *value_to_set);
    if (!prop_set) {
      logger_->log_error("Invalid value was set for property '{}' creating component '{}'", property_name, processor.getName());
      raiseComponentError(processor.getName(), "", prop_set.error().message());
    }
    return;
  }
  logger_->log_warn("Received property {} but is not one of the supported properties for {}. Attempting to add as dynamic property.", property_name, "TODO(mzink)");

  if (!processor.setDynamicProperty(property_name, *value_to_set)) {
    logger_->log_warn("Unable to set the dynamic property {}", property_name);
  } else {
    logger_->log_warn("Dynamic property {} has been set", property_name);
  }
