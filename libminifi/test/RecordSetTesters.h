...
inline void testRecordReader(RecordSetReader& record_set_reader, const std::string_view serialized_record_set, const RecordSet& expected_record_set) {
  const RecordSetFixture fixture;
  ProcessSession& process_session = fixture.processSession();
  ...
  REQUIRE(record_set);
  CHECK(*record_set == expected_record_set);
}
