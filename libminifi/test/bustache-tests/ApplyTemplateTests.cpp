...
TEST_CASE("Test Creation of ApplyTemplate", "[ApplyTemplateCreate]") {
  ...
  std::shared_ptr<core::Processor> processor = std::make_shared<ApplyTemplate>("processorname");
...
TEST_CASE("Test usage of ApplyTemplate", "[ApplyTemplateTest]") {
  ...
  plan->setProperty(getfile, GetFile::Directory, get_file_source_dir);
  ...
  plan->setProperty(maprocessor, ExtractText::Attribute, TEST_ATTR);
  ...
  plan->setProperty(putfile, PutFile::Directory, put_file_destination_dir);
  plan->setProperty(putfile, PutFile::ConflictResolution, magic_enum::enum_name(minifi::processors::PutFile::FileExistsResolutionStrategy::replace));
  ...
  plan->setProperty(atprocessor, ApplyTemplate::Template, template_path);
