...
TEST_CASE("The encrypted flow configuration can be decrypted with the correct key") {
  ...
  CHECK(processor_before->getProperty("HTTP Method").value() == processor_after->getProperty("HTTP Method").value());
  CHECK(processor_before->getProperty("invokehttp-proxy-password").value() == processor_after->getProperty("invokehttp-proxy-password").value());
  ...
  CHECK(controller_service_before->getProperty("CA Certificate").value() == controller_service_after->getProperty("CA Certificate").value());
  CHECK(controller_service_before->getProperty("Passphrase").value() == controller_service_after->getProperty("Passphrase").value());
