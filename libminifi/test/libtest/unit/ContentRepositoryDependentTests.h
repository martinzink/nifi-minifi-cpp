...
struct ReadUntilItCan {
  ...
  minifi::io::ExpectedCallbackReturn operator()(const std::shared_ptr<minifi::io::InputStream> &stream) {
    ...
    while (true) {
      ...
      if (read_result == 0)
        return minifi::io::i64ToExpectedCallbackReturn(gsl::narrow<int64_t>(bytes_read));
...
inline void testErrWrite(std::shared_ptr<core::ContentRepository> content_repo) {
  ...
  REQUIRE_THROWS(
  process_session.write(flow_file, [](const std::shared_ptr<minifi::io::OutputStream>& output_stream) -> minifi::io::ExpectedCallbackReturn {
...
inline void testOkWrite(std::shared_ptr<core::ContentRepository> content_repo) {
  ...
  process_session.write(flow_file, [](const std::shared_ptr<minifi::io::OutputStream>& output_stream) -> minifi::io::ExpectedCallbackReturn {
    std::string str = "new_content";
    return minifi::io::i64ToExpectedCallbackReturn(output_stream->write(as_bytes(std::span(str))));
  });
...
inline void testCancelWrite(std::shared_ptr<core::ContentRepository> content_repo) {
  ...
  process_session.write(flow_file, [](const std::shared_ptr<minifi::io::OutputStream>& output_stream) -> minifi::io::ExpectedCallbackReturn {
    std::string str = "new_content";
    output_stream->write(as_bytes(std::span(str)));
    return nonstd::make_unexpected(MINIFI_IO_CANCEL);
  });
