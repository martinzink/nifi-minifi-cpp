...
struct TestFlow{
  TestFlow(const std::shared_ptr<core::Repository>& ff_repository, const std::shared_ptr<core::ContentRepository>& content_repo, const std::shared_ptr<core::Repository>& prov_repo,
    ...
    process_session_factory_ = std::make_shared<core::ProcessSessionFactory>(processorContext);
 ...
 private:
  ...
  std::shared_ptr<core::ProcessSessionFactory> process_session_factory_;
};
